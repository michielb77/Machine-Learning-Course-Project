...
There are three columns containing timestamp information. I don't think these are relevant for predicting. I might be able to deduce a duration from them, which might in turn help with predicting. But at this point, I am not quite sure what these timestamps even mean. Are they seconds? Milliseconds? What is the offset? I will ignore them for now.

Then there is the column new_window. It only contains "yes" or "no". It is mostly "no" and will only be "yes" if the nu
```{r}
```

```{r}
count(training, new_window)
```

```{r}
training %>% count(user_name, num_window) %>% count(user_name)
```
...
cleanup <- function(data) {
  data %>% select(-(raw_timestamp_part_1:cvtd_timestamp))
}

training <- cleanup(training)
```

The dataset has a large number of columns that have very little variance. To reduce training time, these columns should be removed in preprocessing.
...
training_zonder_classe <- training %>% select(-classe)

pp <- preProcess(training_zonder_classe, method = c("nzv"))
...
# Model
## RPart
I start by exploring some machine learning algorithms such as random forests and glmnet. To reduce training time, but even if I train with only 3 columns, training time is longer then several minutes which I don't like in this stage.
